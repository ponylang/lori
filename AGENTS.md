...
**SSL granularity**: `yield_read()` operates at TCP-read granularity. Every SSL-decrypted message from a single `ssl.receive()` call is delivered inside `_ssl_poll()` before the yield check fires, unless a callback calls `hard_close()` — that stops delivery immediately and drops any messages still undelivered from that read. Per-SSL-message yielding would require changes to `_ssl_poll()` and handling partially-consumed SSL buffers on resume.
