#!/usr/bin/env python3
"""Swarm orchestrator for the lori TCP stress engine (stress-tests/tcp-swarm).
...
independently, so different seeds enable different subsets and push lori's TCP
...
`--binary` at the result (`build/debug/tcp-swarm`); this orchestrator only runs
it, it does not compile. The run mechanism -- the no-progress watchdog (a run is a
...
# with a payload at least that large, a single vectored send then queues more
# buffers than one writev() syscall can send, so lori's `_send_pending_writes()`
# takes its multi-batch path (one writev_max-sized batch per pass). 4 and 64 are
# ordinary small vector writes. COUPLING: the large value must stay above
# PonyTCP.writev_max()'s POSIX return, or the multi-batch coverage silently
# vanishes.
...
# the scheduler threads (an I/O-model change that is deliberately not being made).
# The 65536 payload paired with small read buffers (slow drain) triggers it reliably
# on a real runner; 16384 stays clear of it while keeping the small-read-buffer yield
# coverage. Linux and Windows loopback do not behave this way, so they are uncapped.
# (The separate defect where the pony runtime treated ENOBUFS as fatal was fixed in
# ponylang/ponyc#5823; that fix does not stop the send from blocking.)
...
def main():
    ...
    parser.add_argument("--binary", required=True,
                        help="path to the prebuilt debug engine "
                             "(build/debug/tcp-swarm from `make stress-tests "
